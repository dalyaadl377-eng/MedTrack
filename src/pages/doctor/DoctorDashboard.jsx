...
export default function DoctorDashboard() {
  ...
  // Get the doctor's name directly from AuthContext
  ...
  const stats = useMemo(() => {
    ...
    const reviewed = allReports.filter(
      (r) => r.status === "Reviewed"
    );
  ...
  const insights = useMemo(
    () => generateInsights(patients).slice(0, 3),
    [patients]
  );

  const avgTrend = useMemo(() => {
    const months =
      patients[0]?.trends?.map((t) => t.month) || [];

    return months.map((month, idx) => {
      const withMonth = patients
        .map((p) => p.trends?.[idx])
        .filter(Boolean);
      ...
      return {
        month,
        cholesterol: Math.round(avgChol),
      };
    });
  }, [patients]);
    ...
    <DashboardShell navItems={doctorNav}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <MetricCard
          ...
          value={stats.todaysAppts.length}
          onClick={() =>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            ...
            <table className="w-full text-sm min-w-[520px]">
              <thead>
                <tr className="text-left text-mist-500 text-xs uppercase tracking-wide">
                  ...
                  <th className="font-medium px-2 pb-2 text-right">
                    Actions
                  </th>
                </tr>
              </thead>
              ...
              <tbody>
                {patients
                  ...
                  .map((p) => (
                    <tr
                      key={p.id}
                      className="border-t border-ink-border/60 hover:bg-ink-800/40"
                    >
                      ...
                      <td className="px-2 py-2.5 text-mist-300">
                        {p.condition}
                      </td>

                      <td className="px-2 py-2.5 text-mist-400">
                        {p.lastVisit}
                      </td>
        ...
        <Card className="p-5">
          <h2 className="font-semibold text-mist-100 mb-4">
            Patients by Condition
          </h2>

          <DonutChart
            data={conditionData}
            centerValue={stats.totalPatients}
          ...
          <div className="flex flex-col gap-3">
            {insights.length === 0 && (
              <p className="text-sm text-mist-500">
                No notable trends detected right now.
              </p>
            )}
            ...
            {insights.map((i) => (
              <div
                key={i.id}
                className="rounded-xl bg-ink-800/60 border border-ink-border p-3"
              >
        ...
        <Card className="lg:col-span-3 p-5">
          <h2 className="font-semibold text-mist-100 mb-4">
            Average Cholesterol Trend — All Patients
          </h2>
